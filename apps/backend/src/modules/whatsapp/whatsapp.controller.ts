...
export class WhatsAppController {
  ...
  async handleWebhook(@Req() req: RawBodyRequest<Request>) {
    ...
    try {
      ...
      this.logger.log(`Queued WhatsApp message from ${phone}: "${messageText.substring(0, 50)}..."`);
    } catch (error) {
      this.logger.error(
