...
export class AdminCronService {
  ...
  async monitorSystemHealth() {
    ...
    if (stuckOrders.length > 0) {
      this.logger.warn(`CRITICAL: Found ${stuckOrders.length} orders stuck in PAID state for > 48h. Action required.`);
    ...
    if (chokeSize > 10) {
      this.logger.warn(`SLA BREACH: ${chokeSize} merchants have been sitting in the UNVERIFIED queue for > 24h. Platform adoption blocked.`);
