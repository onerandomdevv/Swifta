...
export class SmsService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!apiKey) {
      this.logger.warn("Africa's Talking API key is missing. SMS functionality will be disabled.");
    } else {
      try {
  ...
  async sendSms(to: string, message: string): Promise<any> {
    if (!this.sms) {
      this.logger.warn(`Skipping SMS to ${to}: Service is poorly configured or disabled.`);
      return;
    }
    ...
    if (!e164Regex.test(to)) {
      this.logger.error(`Failed to send SMS: Invalid phone number format for ${to}. Must be E.164 (e.g. +234...)`);
    ...
    try {
      ...
      this.logger.log(`SMS dispatched to ${to}. MessageId: ${response?.SMSMessageData?.Recipients?.[0]?.messageId}`);
      return response;
    } catch (error: any) {
      this.logger.error(`SMS dispatch failed for ${to}`, error?.response?.data || error.message);
      throw error;
    }
  }
}
