  ...
  private generateInvoiceTable(doc: PDFKit.PDFDocument, order: any) {
    ...
    items.forEach((item, index) => {
      ...
      this.generateTableRow(
        ...
        (Number(item.unitPriceKobo) / 100).toLocaleString('en-NG', { style: 'currency', currency: 'NGN' }),
    ...
    const subtotal = Number(order.totalAmountKobo) - Number(order.deliveryFeeKobo);
    ...
    this.generateTableRow(
      ...
      'Subtotal',
      '',
      (subtotal / 100).toLocaleString('en-NG', { style: 'currency', currency: 'NGN' })
    );
    ...
    this.generateTableRow(
      ...
      'Delivery Fee',
      '',
      (Number(order.deliveryFeeKobo) / 100).toLocaleString('en-NG', { style: 'currency', currency: 'NGN' })
    );
    ...
    this.generateTableRow(
      ...
      'Total Paid',
      '',
      (Number(order.totalAmountKobo) / 100).toLocaleString('en-NG', { style: 'currency', currency: 'NGN' })
    );
    doc.font('Helvetica');
  }
