      ...
      <div className="flex items-center gap-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-slate-500 font-bold text-sm tracking-wide mt-2">
            Finalize payment for your order
          </p>
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {[
                ...
                <button
                  key={method.id}
                  className={`p-6 rounded-[2rem] border-2 transition-all text-center space-y-3 ${method.active ? "border-navy-dark bg-slate-50 dark:bg-slate-800" : "border-slate-100 dark:border-slate-800 hover:border-slate-200 opacity-50 cursor-not-allowed"}`}
                  disabled={!method.active}
                >
                  <span
                    className={`material-symbols-outlined text-3xl ${method.active ? "text-navy-dark dark:text-white" : "text-slate-300"}`}
                  >
                    {method.icon}
                  </span>
                  <p
                    className={`text-[10px] font-black uppercase tracking-widest ${method.active ? "text-navy-dark dark:text-white" : "text-slate-400"}`}
                  >
                    {method.label}
                  </p>
                </button>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm sticky top-10">
            ...
            <div className="pt-8 border-t border-slate-50 dark:border-slate-800 mb-10">
              <div className="flex justify-between items-end">
                ...
                <p className="text-4xl font-black text-navy-dark dark:text-white tracking-tighter uppercase leading-none">
                  {formatKobo(total)}
                </p>
              </div>
            </div>
            ...
            <p className="mt-8 text-center text-[9px] font-black uppercase tracking-widest text-slate-400 opacity-60 px-6 leading-relaxed">
              By placing the order, you agree to our terms of escrow and
