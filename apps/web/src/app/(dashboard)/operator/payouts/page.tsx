...
export default function OperatorPayoutsPage() {
  ...
  const { data: payouts, isLoading } = useQuery<PayoutRequest[]>({
    queryKey: ["operator", "payouts", "pending"],
    queryFn: () => apiClient.get("/admin/payouts"), // Uses the same backend endpoint
  });
  ...
  return (
    <div className="space-y-8 animate-in mt-4 fade-in slide-in-from-bottom-4">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-slate-800 uppercase tracking-widest">
            Payout Requests
          </h1>
        ...
        <div className="flex items-center gap-4">
          <div className="bg-orange-50 text-orange-600 font-bold px-4 py-2.5 rounded-xl text-sm border-2 border-orange-100">
            <span className="mr-2 text-slate-600">Pending Requests:</span>
            {payouts?.length || 0}
          </div>
        </div>
      </header>
      ...
      {/* Main Table View */}
      <section className="bg-white border-2 border-slate-100 rounded-[2rem] shadow-sm overflow-hidden">
                  ...
                  >
                    ...
                    <td className="p-4 md:p-6 text-right whitespace-nowrap">
                      <Button
                        size="sm"
                        variant="ghost"
                        className="text-orange-500 hover:bg-orange-50 font-bold tracking-wider uppercase text-xs"
                      >
                        Review
                      </Button>
                    </td>
      ...
      {selectedPayout && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-[2rem] w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 animate-in slide-in-from-bottom-8">
            ...
            <div className="p-6 md:p-8 space-y-8">
              ...
              {/* Merchant Details */}
              <div>
                <h3 className="text-xs font-black text-orange-500 uppercase tracking-widest mb-4">
                  Merchant Details
                </h3>
                ...
                <div className="bg-slate-50 p-6 rounded-xl border border-slate-100">
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="font-black text-orange-500 tracking-widest text-xl">
            ...
            <div className="p-6 md:p-8 border-t border-slate-100 bg-slate-50 flex gap-4 justify-end rounded-b-[2rem]">
              <Button
                variant="outline"
                className="w-full md:w-auto h-12 text-sm font-black tracking-widest uppercase hover:bg-slate-100 text-slate-600"
                onClick={() => setSelectedPayout(null)}
                disabled={processMutation.isPending}
              >
                Cancel
              ...
              <Button
                className="w-full md:w-auto h-12 text-sm font-black tracking-widest uppercase bg-orange-500 hover:bg-orange-600 text-white shadow-lg shadow-orange-500/20"
