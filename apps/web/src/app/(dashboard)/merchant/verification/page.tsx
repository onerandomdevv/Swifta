...
export default function MerchantVerificationPage() {
  ...
  const handleTier2Submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!ninNumber || !idUrl)
      return toast.error("Please provide NIN and ID document");
  ...
  const handleTier3Submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cacUrl || !addressUrl)
      return toast.error("Please provide CAC and Proof of Address");
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 md:py-12 bg-background min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div className="space-y-2">
          ...
          <p className="text-foreground-muted text-sm md:text-base max-w-xl">
            Upgrade your business tier to unlock higher limits, lower fees, and
            premium trust badges.
          </p>
        </div>
        <div className="bg-surface p-4 rounded-2xl border border-border shadow-sm flex items-center gap-4">
          <div className="flex flex-col shrink-0">
            <span className="text-[10px] font-bold text-foreground-muted uppercase tracking-widest mb-1">
              Current Tier
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-surface rounded-2xl border border-border p-6 shadow-sm sticky top-24">
            <h3 className="text-sm font-black uppercase tracking-widest text-foreground-muted mb-8">
            ...
            <div className="mt-8 pt-8 border-t border-border">
              <h4 className="text-xs font-bold text-foreground uppercase mb-4">
                Tier Benefits
              </h4>
              <ul className="space-y-3">
                ...
                />
                <BenefitItem
                  label="Lowest Platform Fees (1%)"
                  tier={3}
                  current={status.currentTier}
                />
                <BenefitItem
                  label="Business Trusted Badge"
                  tier={3}
                  current={status.currentTier}
                />
                <BenefitItem
                  label="Priority Bulk Logistics"
                  tier={3}
                  current={status.currentTier}
                />
              </ul>
        ...
        <div className="lg:col-span-8">
          {activeTier === 1 && (
                ...
                status.tier1.status === "COMPLETE" ? (
                  <div className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 p-4 rounded-xl flex items-center gap-3 border border-emerald-100 dark:border-emerald-800">
                    ...
                    <span className="text-sm font-bold">
                      You have completed all Level 1 requirements.
                    </span>
                  </div>
                ) : (
                  <button
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <StatusItem
                  label="Email Address"
                  met={status.tier1.requirements.emailVerified}
                />
                <StatusItem
                  label="Phone Number"
                  met={status.tier1.requirements.phoneVerified}
                />
                <StatusItem
                  label="Bank Account"
                  met={status.tier1.requirements.bankVerified}
                />
                <StatusItem
                  label="Business Address"
                  met={status.tier1.requirements.businessAddress}
                />
                <StatusItem
                  label="Basic Profile Info"
                  met={status.tier1.requirements.basicInfo}
                />
              </div>
            </TierDetailView>
          )}
          ...
          {activeTier === 2 && (
            <TierDetailView
              ...
              footer={
                status.tier2.status === "COMPLETE" ? null : status.tier2
                    .status === "LOCKED" ? (
                  <div className="bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 p-4 rounded-xl flex items-center gap-3 border border-amber-100 dark:border-amber-800">
                    ...
                    <span className="text-sm font-bold">
                      Complete Level 1 requirements first to unlock this tier.
                    </span>
                  </div>
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 p-6 rounded-xl space-y-4 border border-blue-100 dark:border-blue-800">
                    ...
                    <p className="text-xs leading-relaxed opacity-80">
                      Your identity documents are being verified by our
                      compliance team. This typically takes 24 business hours.
                    </p>
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        >
                          <option value={VerificationIdType.NIN}>
                            National ID (NIN)
                          </option>
                          <option value={VerificationIdType.VOTERS_CARD}>
                            Voter's Card
                          </option>
                          <option value={VerificationIdType.PASSPORT}>
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        {uploading === "id" ? (
                          ...
                          <>
                            <span className="material-symbols-outlined text-emerald-500 font-variation-fill">
                              check_circle
                            </span>
                          ...
                          <>
                            <span className="material-symbols-outlined text-foreground-muted opacity-30 text-3xl">
                              upload_file
                            </span>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <StatusItem
                  label="Account Identity (ID/NIN)"
                  met={status.tier2.requirements.ninVerified}
                />
                <StatusItem
                  label="Completed Orders"
                  met={
                    status.tier2.requirements.completedOrders.current >=
                    status.tier2.requirements.completedOrders.required
                  }
                  value={`${status.tier2.requirements.completedOrders.current}/${status.tier2.requirements.completedOrders.required}`}
                />
                <StatusItem
                  label="Dispute-Free History"
                  met={status.tier2.requirements.zeroDisputes}
                />
                <StatusItem
                  label="Profile Photo Set"
                  met={status.tier2.requirements.profilePhoto}
                />
              </div>
            </TierDetailView>
          )}
          ...
          {activeTier === 3 && (
            <TierDetailView
              ...
              footer={
                status.tier3.status === "COMPLETE" ? null : status.tier3
                    .status === "LOCKED" ? (
                  <div className="bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 p-4 rounded-xl flex items-center gap-3 border border-amber-100 dark:border-emerald-800/20">
                    <span className="material-symbols-outlined">lock</span>
                    <span className="text-sm font-bold">
                      You need to reach Level 2 before applying for Business
                      Verification.
                    </span>
                  </div>
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 p-6 rounded-xl space-y-4 border border-blue-100 dark:border-blue-800">
                    ...
                    <p className="text-xs leading-relaxed opacity-80">
                      We are reviewing your CAC documentation and warehouse
                      address. This elite verification level usually takes 48-72
                      business hours.
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            {uploading === "cac" ? (
                              ...
                              <span className="text-[10px] font-bold text-foreground-muted opacity-40">
                                SELECT FILE
                              </span>
                            )}
                          </div>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <StatusItem
                  ...
                  met={status.tier3.requirements.cacVerified}
                />
                <StatusItem
                  label="Physical Address Verified"
                  met={status.tier3.requirements.addressVerified}
                />
                <StatusItem
                  label="Completed Orders"
                  met={
                    status.tier3.requirements.completedOrders.current >=
                    status.tier3.requirements.completedOrders.required
                  }
                ...
                <StatusItem
                  ...
                  met={
                    (status.tier3.requirements.averageRating.current || 0) >=
                    status.tier3.requirements.averageRating.required
                  }
                ...
                <StatusItem
                  ...
                  met={
                    status.tier3.status === "COMPLETE" ||
                    status.tier3.requirements.accountAge.current.includes(
                      "month",
                    )
                  }
      ...
      <div className="mt-20 p-8 border-2 border-dashed border-border rounded-3xl flex flex-col md:flex-row items-center gap-8 bg-surface/50">
        <div className="size-20 bg-primary/10 rounded-2xl flex items-center justify-center shrink-0">
          <span className="material-symbols-outlined text-4xl text-primary font-variation-fill">
            shield_person
          </span>
        </div>
...
function TierNavButton({
  ...
  return (
    ...
    >
      <span
        className={cn(
          "material-symbols-outlined text-xl font-variation-fill",
          colorMap[status],
        )}
      >
        {iconMap[status]}
...
function BenefitItem({
  ...
  const isUnlocked =
    current === `TIER_${tier}` || (tier === 2 && current === "TIER_3");
  return (
    <li
      className={cn(
    ...
    >
      <span className="material-symbols-outlined text-sm font-variation-fill">
        verified
      </span>
      {label}
    </li>
...
function TierDetailView({
  ...
  return (
    <div className="bg-surface rounded-3xl border border-border p-8 shadow-sm space-y-8 animate-in fade-in duration-500 relative overflow-hidden">
      {isLocked && (
        <div className="absolute inset-0 z-10 bg-surface/60 backdrop-blur-[1px] flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500">
          <div className="size-16 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center border border-slate-100 dark:border-slate-700 mb-4 shadow-sm">
            <span className="material-symbols-outlined text-3xl text-slate-400 font-variation-fill">
              lock
            </span>
          </div>
          <h4 className="text-lg font-black uppercase tracking-tight text-foreground dark:text-white mb-2">
            Requirement Locked
          </h4>
          <p className="text-sm text-foreground-muted max-w-xs mx-auto font-medium">
            You must complete the previous level fully before these requirements
      ...
      {footer && <div className="pt-8 border-t border-border">{footer}</div>}
    </div>
  );
}
