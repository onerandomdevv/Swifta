...
export default function MerchantAnalyticsPage() {
        ...
        async function load() {
            try {
                const [orderData, rfqData] = await Promise.all([
                    ...
                    getMerchantRFQs(1, 100) as unknown as Promise<RFQ[]>,
                ]);
    ...
    const acceptanceRate = totalRfqs > 0 ? Math.round((completedOrders / totalRfqs) * 100) : 0;

    const kpis = [
        { label: 'Pipeline Value', value: formatKobo(pipelineValue), trend: `${orders.length} orders`, trendType: 'up', subtext: 'Total order value', icon: 'info' },
        { label: 'Acceptance Rate', value: `${acceptanceRate}%`, trend: `${completedOrders}/${totalRfqs}`, trendType: 'up', subtext: 'Completed vs RFQs', icon: 'query_stats' },
        { label: 'Open RFQs', value: `${rfqs.filter(r => r.status === 'OPEN').length}`, trend: `${quotedRfqs} quoted`, trendType: 'up', subtext: 'Awaiting response', icon: 'timer' },
    ];
                    ...
                    <div key={idx} className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all group">
                        ...
                        {kpi.label === 'Acceptance Rate' && (
                            <div className="mt-8 h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                <div className="bg-primary-navy h-full rounded-full" style={{ width: `${acceptanceRate}%` }}></div>
                            </div>
                        )}
                    </div>
                ))}
                ...
                <main className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-2xl shadow-navy-dark/5 overflow-hidden">
                        <div className="p-8 border-b border-slate-50 dark:border-slate-800">
                            <h3 className="text-xl font-black text-navy-dark dark:text-white tracking-tight uppercase">Order History</h3>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                                    {orders.length === 0 ? (
                                        <tr>
                                            <td colSpan={4} className="px-8 py-12 text-center text-slate-400 text-sm font-bold uppercase tracking-widest">No orders yet</td>
                                        </tr>
                                    ) : (
                                        orders.slice(0, 10).map((order) => (
                                            <tr key={order.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all duration-300">
                                                ...
                                                <td className="px-8 py-6">
                                                    <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">{new Date(order.createdAt).toLocaleDateString()}</p>
                                                </td>
                                                <td className="px-8 py-6">
                ...
                <aside className="space-y-8 sticky top-24">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 p-8 shadow-sm">
                        <div className="flex items-center gap-3 mb-8">
                            <span className="material-symbols-outlined text-primary-navy font-black">analytics</span>
                            <h4 className="text-sm font-black text-navy-dark dark:text-white uppercase tracking-widest">Summary</h4>
                        </div>
                        <div className="space-y-6">
                            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                                <span className="text-slate-500">Total Orders</span>
                                <span className="text-navy-dark dark:text-white">{orders.length}</span>
                            </div>
                            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                            ...
                            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                                ...
                                <span className="text-navy-dark dark:text-white">{totalRfqs}</span>
                            </div>
                            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                                <span className="text-slate-500">Revenue</span>
                                <span className="text-navy-dark dark:text-white">{formatKobo(pipelineValue)}</span>
                            </div>
                        </div>
                    </div>
                </aside>
