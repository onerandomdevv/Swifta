...
export default function MerchantOrdersPage() {
  const stats = [
    { label: 'Total Active Orders', value: '1,284', trend: '+12.5%', trendColor: 'text-emerald-600' },
    { label: 'Pending Payment', value: '42', badge: 'High Alert', badgeColor: 'bg-amber-100 text-amber-700' },
    { label: 'Ready for Dispatch', value: '15', trend: 'Steady', trendColor: 'text-slate-500' },
    { label: 'Revenue (Today)', value: '₦2.48M', trend: '+18.2%', trendColor: 'text-emerald-600' },
  ];
      ...
      <main className="flex-1 space-y-8 py-8 px-4 md:px-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            <h1 className="text-4xl font-black text-navy-dark dark:text-white tracking-tight uppercase leading-none font-display">Orders Management</h1>
            <p className="text-slate-500 font-bold text-sm tracking-wide mt-2 uppercase opacity-70">Manage and track your B2B hardware trades across Lagos districts.</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, idx) => (
            <div key={idx} className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">{stat.label}</p>
              <div className="flex items-baseline gap-3">
                <h3 className="text-3xl font-black text-navy-dark dark:text-white tracking-tight leading-none uppercase">{stat.value}</h3>
                {stat.trend && (
                  <span className={`text-[9px] font-black tracking-widest ${stat.trendColor}`}>
                    {stat.trend}
                  </span>
                )}
              </div>
              {stat.badge && (
                <div className="mt-4">
                  <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${stat.badgeColor}`}>
                    {stat.badge}
                  </span>
                </div>
              )}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-2xl shadow-navy-dark/5 overflow-hidden">
          ...
          <div className="px-8 py-5 border-b border-slate-50 dark:border-slate-800 bg-slate-50/30 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl">
              {['All Orders', 'Pending', 'Paid', 'Dispatched', 'Completed'].map((tab, i) => (
                <button
                  key={tab}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800">
                  ...
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Customer & Location</th>
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Order Date</th>
                ...
                {orders.map((order, idx) => (
                  <tr key={idx} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all duration-300">
                    <td className="px-8 py-8">
                      <p className="text-sm font-black text-navy-dark dark:text-white uppercase leading-tight">#{order.id}</p>
                    </td>
                    <td className="px-8 py-8">
                      <div className="space-y-1">
                        <p className="text-sm font-black text-navy-dark dark:text-white">{order.customer}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{order.location}</p>
                      </div>
                    </td>
                    <td className="px-8 py-8">
                      <p className="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-tight">{order.date}</p>
                    </td>
                    <td className="px-8 py-8">
                      <p className="text-sm font-black text-navy-dark dark:text-white tracking-tight">{order.amount}</p>
                    </td>
                    <td className="px-8 py-8">
                      <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border h-7 ${order.status === 'PENDING_PAYMENT' ? 'bg-amber-50 text-amber-700 border-amber-100' :
                          order.status === 'PAID' ? 'bg-blue-50 text-blue-700 border-blue-100' :
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between px-8 py-6 border-t border-slate-50 dark:border-slate-800 bg-slate-50/30">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 sm:mb-0">
              Showing <span className="text-navy-dark dark:text-white">1-6</span> of <span className="text-navy-dark dark:text-white">1,284 orders</span>
            </p>
        ...
        <div className="pt-8 pb-4 text-center">
          <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em]">© 2023 Hardware OS Lagos • Secured B2B Trading Platform</p>
        </div>
      </main>
    </div>
  );
