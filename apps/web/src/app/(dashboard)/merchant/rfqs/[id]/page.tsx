...
export default function MerchantRFQDetailsPage() {
  ...
  const handleQuote = async () => {
    ...
    console.log('Strict Compliance Check: Submitting bid in Kobo:', bidInKobo.toString());
    ...
    <div className="space-y-10 py-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center gap-6">
        ...
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-navy-dark dark:text-white tracking-tight uppercase leading-none font-display">Contract Tender</h1>
          <p className="text-slate-500 font-bold text-sm tracking-wide mt-2">Buyer: {rfq.buyer} • Project: {rfq.project}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-7 space-y-10">
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm relative overflow-hidden group">
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-50 dark:border-slate-800">
              <h3 className="text-sm font-black text-navy-dark dark:text-white uppercase tracking-widest">Material List</h3>
              ...
              {rfq.items.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Requested Qty: {item.qty} units</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Buyer Estimate</p>
                    <p className="text-sm font-black text-navy-dark dark:text-white tabular-nums">{formatKobo(item.estPrice)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-slate-50/50 dark:bg-slate-800/30 border border-dashed border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-10">
            <div className="flex items-center gap-4 mb-4">
              <span className="material-symbols-outlined text-blue-500 font-black">info</span>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Quote Expiration Notice</p>
            </div>
            <p className="text-xs font-bold text-slate-500 leading-relaxed italic">
              Quotes submitted on Hardware OS are binding for 48 hours. Ensure your logistics capacity is verified before finalizing your bid. Settlement occurs via escrow upon delivery confirmation.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-5">
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm sticky top-10">
            <h3 className="text-sm font-black text-navy-dark dark:text-white uppercase tracking-widest mb-10 pb-4 border-b border-slate-50 dark:border-slate-800">Submit Your Quote</h3>

            <div className="space-y-8">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Total Settlement (₦)</label>
                <div className="relative group">
                  <span className="absolute left-6 top-1/2 -translate-y-1/2 font-black text-navy-dark dark:text-white">₦</span>
                  <input
                    type="number"
                    value={bidAmount}
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Estimated Delivery</label>
