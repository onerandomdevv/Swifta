...
export default function MerchantQuotesPage() {
            ...
            try {
                ...
                const results = await Promise.all(
                    rfqList.map(async (rfq) => {
                        try {
                            const quotes = (await getQuotesByRFQ(rfq.id)) as unknown as Quote[];
                            return (Array.isArray(quotes) ? quotes : []).map((q) => ({ quote: q, rfq }));
                        } catch {
                            return [];
                        }
                    })
    ...
    const activeCount = quotesWithRfqs.filter(q => q.quote.status === 'PENDING').length;
    const acceptedCount = quotesWithRfqs.filter(q => q.quote.status === 'ACCEPTED').length;
    const totalValue = quotesWithRfqs.reduce((sum, q) => sum + BigInt(q.quote.totalPriceKobo || 0), 0n);

    const stats = [
        { label: 'Active Quotes', value: String(quotesWithRfqs.length), trend: `${activeCount} pending`, trendColor: 'text-emerald-600' },
        { label: 'Accepted', value: String(acceptedCount), trend: 'Converted to orders', trendColor: 'text-emerald-600' },
        { label: 'Pipeline Value', value: formatKobo(totalValue), trend: `${quotesWithRfqs.length} quotes`, trendColor: 'text-emerald-600' },
    ];
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {quotesWithRfqs.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-8 py-16 text-center text-slate-400 text-sm font-bold uppercase tracking-widest">
                                ...
                                quotesWithRfqs.map(({ quote, rfq }) => {
                                    ...
                                    return (
                                        <tr key={quote.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all duration-300">
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-5">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-black text-navy-dark dark:text-white leading-tight mb-1">#{quote.id.slice(0, 8)}</p>
                                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">RFQ: #{rfq.id.slice(0, 8)}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6">
                                                <p className="text-sm font-black text-navy-dark dark:text-white tracking-tight">{formatKobo(BigInt(quote.totalPriceKobo))}</p>
                                            </td>
                                            <td className="px-8 py-6">
                                                <p className="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">{new Date(quote.createdAt).toLocaleDateString()}</p>
                                            </td>
                                            <td className="px-8 py-6">
                                                <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border h-7 ${
                                                    status === 'Accepted' ? 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-900/10 dark:text-emerald-400' :
                                                    status === 'Sent' ? 'bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-900/10 dark:text-blue-400' :
                                                    status === 'Declined' ? 'bg-red-50 text-red-600 border-red-100 dark:bg-red-900/10 dark:text-red-400' :
                                                    ...
                                                    <span className={`size-1.5 rounded-full mr-2 ${
                                                        ...
                                                        status === 'Declined' ? 'bg-red-500' : 'bg-slate-300'
                                                    }`}></span>
                                                    {status}
                                                </span>
                                            </td>
